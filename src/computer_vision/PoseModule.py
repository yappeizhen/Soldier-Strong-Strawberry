...
class poseDetector() :
    ...
    def findAngle(self, img, p1, p2, p3, draw=True):   
        ...
        if draw:
            ...
            cv2.circle(img, (x1, y1), 5, circle_color, cv2.FILLED)
            cv2.circle(img, (x1, y1), 15, circle_color, 2)
            cv2.circle(img, (x2, y2), 5, circle_color, cv2.FILLED)
            cv2.circle(img, (x2, y2), 15, circle_color, 2)
            cv2.circle(img, (x3, y3), 5, circle_color, cv2.FILLED)
            cv2.circle(img, (x3, y3), 15, circle_color, 2)
            
            cv2.putText(img, str(int(angle)), (x2-50, y2+50), 
                        cv2.FONT_HERSHEY_PLAIN, 2, circle_color, 2)
        return angle
